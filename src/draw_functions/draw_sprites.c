...
// 		angle_x -= (float)M_PI * 2;
// 	fov_left = game->chars[0]->pa + (game->menu->settings->fov / 2) - angle_x;
...
// 		fov_left -= (float)M_PI * 2;
// 	screen_pos[0] = fov_left * (WIDTH / game->menu->settings->fov);
// 	screen_pos[1] = HEIGHT / 2;
// 	// printf("%f, %f\n", screen_pos[0], screen_pos[1]);
// 	if (screen_pos[0] < WIDTH && screen_pos[1] < HEIGHT && screen_pos[0] > 1 && screen_pos[1] > 1)
// 	{
// 		mlx_put_pixel(game->game_img, screen_pos[0], screen_pos[1], 0xFF0000FF);
// 		mlx_put_pixel(game->game_img, screen_pos[0] - 1, screen_pos[1] - 1, 0xFF0000FF);
// 		mlx_put_pixel(game->game_img, screen_pos[0] - 1, screen_pos[1], 0xFF0000FF);
// 		mlx_put_pixel(game->game_img, screen_pos[0], screen_pos[1] + 1, 0xFF0000FF);
// 		mlx_put_pixel(game->game_img, screen_pos[0] + 1, screen_pos[1] + 1, 0xFF0000FF);
// 		mlx_put_pixel(game->game_img, screen_pos[0] + 1, screen_pos[1], 0xFF0000FF);
// 		mlx_put_pixel(game->game_img, screen_pos[0], screen_pos[1] + 1, 0xFF0000FF);
// 	}
...
// 	if (game->chars[0]->pa > ((float)M_PI * 2 / 3) && angle_x < (float)(M_PI / 2))
// 		screen_pos[1] = game->chars[0]->pa + (game->menu->settings->graphics / 2) - angle_x - ((float)M_PI * 2);
...
void	draw_sprite(t_game *game)
{
	...
	while (i < 100)
	{
		...
		while (j < 100)
		{
			if (screen_pos[0] - i < WIDTH && screen_pos[1] - j < HEIGHT && screen_pos[0] - i > 1 && screen_pos[1] - j > (HEIGHT / 2) && b < game->rays[(int)screen_pos[0] - i])
